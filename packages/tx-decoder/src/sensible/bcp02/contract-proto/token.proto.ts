...
export function getQueryCodehash(script: Buffer): string {
  return getContractCodeHash(script).toString("hex");
}

export function getQueryGenesis(script: Buffer): string {
  return getTokenID(script).toString("hex");
}
...
export function getQuerySensibleID(script0: Buffer): string {
  ...
  return sensibleIDBuf.toString("hex");
}
...
export function parseScript(
  ...
  const dataPart = parseDataPart(scriptBuf);
  const tokenAddress = bsv.Address.fromPublicKeyHash(
    Buffer.from(dataPart.tokenAddress, "hex"),
    network
  ).toString();
  const genesis = getQueryGenesis(scriptBuf);
  const codehash = getQueryCodehash(scriptBuf);
  const sensibleId = getQuerySensibleID(scriptBuf);
  return {
    codehash,
    genesis,
    sensibleId,
